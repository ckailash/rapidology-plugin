  ...
  function clear_stats_single_optin() {
					...
					if ( 'true' == $details['is_authorized'] ) {
						...
						switch ( $service ) {
							case 'mailchimp' :
								...
								$error_message = $mailchimp->get_mailchimp_lists( $details['api_key'], $name );
								break;
	...
	function authorize_account() {
		...
		switch ( $service ) {
			...
			case 'hubspot' :
				$hubspot = new rapidology_hubspot();
				$error_message = $hubspot->get_hubspot_lists( $api_key, $name );
				break;
			case 'hubspot-standard' :
				$hubspot_standard = new rapidology_hubspot_standard();
				$error_message = $hubspot_standard->get_hubspot_forms($account_id, $api_key, $name);
				break;
			case 'salesforce' :
				$salesforce = new rapidology_salesforce();
		...
		if ( is_email( $email ) ) {
			...
			switch ( $service ) {
				case 'mailchimp' :
					...
					$mailchimp = new rapidology_mailchimp();
					$error_message = $mailchimp->subscribe_mailchimp( $api_key, $list_id, $email, $name, $last_name, $dbl_optin );
					break;
				case 'hubspot' :
					$api_key = $options_array['accounts'][$service][$account_name]['api_key'];
					$hubspot = new rapidology_hubspot();
					$error_message = $hubspot->hubspot_subscribe($api_key, $email, $list_id, $name, $last_name);
					break;
				case 'hubspot-standard' :
					$api_key = $options_array['accounts'][$service][$account_name]['api_key'];
				...
				case 'salesforce' :
					...
					$token			= $options_array['accounts'][ $service ][ $account_name ]['token'];
					$salesforce = new rapidology_salesforce();
					$error_message = $salesforce->subscribe_salesforce($url, $version, $client_key, $client_secret, $username_sf, $password_sf, $token, $name, $last_name, $email, $list_id);
				break;
				case 'activecampaign':
					$api_key    = $options_array['accounts'][ $service ][ $account_name ]['api_key'];
					$url		= $options_array['accounts'][ $service ][ $account_name ]['url'];
